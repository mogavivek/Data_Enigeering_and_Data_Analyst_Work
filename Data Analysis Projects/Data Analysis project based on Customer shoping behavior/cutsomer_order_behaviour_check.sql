...
from df_customer
where discount_applied = 'Yes' and purchase_amount >= (select AVG(purchase_amount) from df_customer)
...
/* If we use dense rank then it will put the same row number if the count is same, that is why row number used */
